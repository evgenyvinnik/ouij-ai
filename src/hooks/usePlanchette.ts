...
export function usePlanchette() {
  const { planchette, movePlanchette, setOffset, setDragging } = useOuijaStore();
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    const x = ((e.clientX - rect.left - planchette.offset.x) / rect.width) * 100;
    const y = ((e.clientY - rect.top - planchette.offset.y) / rect.height) * 100;
...
export function useMovePlanchetteTo() {
  ...
  return useCallback((target: Position, duration: number = 1000): Promise<void> => {
